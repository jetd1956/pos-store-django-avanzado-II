...
class Sale(models.Model):
    ...
    subtotal_con_iva = models.DecimalField(max_digits=9, decimal_places=2, default=0.00, verbose_name='Subtotal c/Iva')
    subtotal_sin_iva = models.DecimalField(max_digits=9, decimal_places=2, default=0.00, verbose_name='Subtotal s/Iva')
    ...
    def get_full_subtotal(self):
        return float(self.subtotal_sin_iva) + float(self.subtotal_con_iva)

    def calculate_invoice(self):
        self.subtotal_sin_iva = float(self.saledetail_set.filter(product__with_tax=False).aggregate(result=Coalesce(Sum('total'), 0.00, output_field=FloatField())).get('result'))
        self.subtotal_con_iva = float(self.saledetail_set.filter(product__with_tax=True).aggregate(result=Coalesce(Sum('total'), 0.00, output_field=FloatField())).get('result'))
    ...
    def toJSON(self):
        ...
        item['subtotal_con_iva'] = float(self.subtotal_con_iva)
        item['subtotal_sin_iva'] = float(self.subtotal_sin_iva)
